...
// ✅ Ensure correct font config
const pacifico = Pacifico({
  weight: ['400'], // string array
  subsets: ['latin'],
  display: 'swap',
});
...
export default function DashboardLayout({ children }) {
  ...
  useEffect(() => {
    if (pathname === '/home/profile') setActiveTab(null);
    else if (pathname.startsWith('/home/dashboard')) setActiveTab('Home');
    else if (pathname.startsWith('/home/create-post')) setActiveTab('Create Post');
    else if (pathname.startsWith('/home/analytics')) setActiveTab('Analytics');
    else if (pathname.startsWith('/home/manage-comments')) setActiveTab('Manage Comments');
    else if (pathname.startsWith('/home/posts-folder')) setActiveTab('Posts Folder');
    else if (pathname.startsWith('/home/manage-accounts')) setActiveTab('Manage Account');
    else if (pathname.startsWith('/home/workspace')) setActiveTab('Workspace');
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans">
      ...
      >
        ...
        <div>
          ...
          <nav className="space-y-1">
            ...
            {sidebarOpen && <hr className="my-4 border-gray-200" />}
            {!sidebarOpen && <div className="py-2"></div>}
      ...
      <main className="flex-1 p-6 overflow-y-auto bg-white rounded-l-2xl shadow-inner">
        {children}
      </main>
    </div>
  );
